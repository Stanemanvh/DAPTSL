...
from .util.data import SamplerType, make_data_loader, make_dataset
from .util.data_transforms import get_transform
from .util.logging import MetricLogger
from .eval_metrics import MetricType, build_metric
from .eval_setup import setup_and_build_model, get_args_parser as get_setup_args_parser
from .eval_helpers import ModelWithIntermediateLayers, evaluate
...
def get_args_parser(
    ...
    parser.add_argument("--wandb", type=str, default=None, help="Wandb project name, eg: explora_linprobe")
    parser.add_argument("--wandb_entity", type=str, default="TODO: FILL IN", help="Wandb entity name")
    parser.add_argument(
        "--wandb_logdir",
        type=str,
        default="./wandb",
    ...
    parser.set_defaults(
        train_dataset_str="fmow:data_and_checkpoints/fmow_csvs/train_62classes.csv",
        val_dataset_str="fmow:data_and_checkpoints/fmow_csvs/test_62classes.csv",
        test_dataset_strs=None,
        epochs=10,
        batch_size=256,
        num_workers=8,
        epoch_length=1500,
        save_checkpoint_frequency=1500,
        eval_period_iterations=1500,
        learning_rates=[1e-4, 1e-3, 5e-3, 1e-2, 2e-2, 5e-2, 0.1],
...
def evaluate_linear_classifiers(
    ...
    metric = build_metric(metric_type, num_classes=num_classes, ks=(1, 5) if num_classes > 5 else (1,))
...
def eval_linear(
    *,
    ...
    for batch_sample in metric_logger.log_every(
        train_data_loader,
        10,
        header,
        max_iter,
        ...
        data, labels = batch_sample[0], batch_sample[1]
        ...
        if iteration % 10 == 0:
            ...
            if wandb_project is not None and distributed.is_main_process():
                log_lr = optimizer.param_groups[0]["lr"]
                module_without_ddp = linear_classifiers.module if distributed.is_enabled() else linear_classifiers
                n_parameters_dict = {
                    k: sum(p.numel() for p in v.parameters() if p.requires_grad)
                    for k, v in module_without_ddp.classifiers_dict.items()
                }
                try:
                    wandb.log(
                        {
        ...
        if eval_period > 0 and (iteration + 1) % eval_period == 0 and iteration != max_iter - 1:
            results_dict = evaluate_linear_classifiers(
            ...
            if wandb_project is not None and distributed.is_main_process():
                try:
                    wandb.log({"train_step": iteration} | results_dict)
                except ValueError:
                    pass
            torch.cuda.synchronize()
    ...
    if wandb_project is not None and distributed.is_main_process():
        try:
            wandb.log({"train_step": iteration} | val_results_dict)
        except ValueError:
            pass
...
def make_eval_data_loader(test_dataset_str, batch_size, num_workers, metric_type):
    eval_transform = get_transform(test_dataset_str, is_train=False)
...
def run_eval_linear(
    ...
    train_transform = get_transform(train_dataset_str, is_train=True)
    ...
    training_num_classes = (
        ...
        if hasattr(train_dataset, "get_targets")
        else train_dataset.n_classes
    )
    ...
    val_data_loader = make_eval_data_loader(val_dataset_str, batch_size, 0, val_metric_type)
...
def main(args):
    model, cfg, autocast_dtype = setup_and_build_model(args)
    # Set up wandb
